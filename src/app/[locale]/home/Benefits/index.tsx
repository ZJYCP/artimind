...
const Benefits: FC<PropsWithChildren<ComProps>> = (props) => {
  ...
  return (
    <section className="py-16">
      <h2 className="text-3xl font-bold text-center mb-12 text-blue-300">{t("benefitsTitle")}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {benefitsMap.map((benefit) => (
          <div key={benefit.title} className={`p-6 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} hover:transform hover:scale-105 transition duration-300`}>
            {benefit.icon}
            <h3 className="text-xl font-semibold mb-2 text-blue-300">{benefit.title}</h3>
